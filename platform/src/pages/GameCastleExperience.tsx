  ...
  return <main className={`gc-app gc-${phase} chaos-${chaos}`}>
    {phase === "world" && <section className="world-screen">
      ...
      <header className="world-head">
        ...
        <div className="world-links"><button type="button" onClick={() => setDrawer("remix")}><span>▶</span> PLAY GAMES</button><button type="button" onClick={() => setDrawer("director")}><span>✦</span> AI DIRECTOR</button></div>
      </header>
      ...
      <button className="hotspot hotspot-eye" type="button" onClick={() => setDrawer("director")}><i /> THE EYE IS OPEN</button>
      ...
      <form className="idea-console" onSubmit={submit}>
        <div className="console-top"><span>{remixSource ? `REMIXING ${remixSource}` : "WHAT ARE WE MAKING?"}</span><button type="button" onClick={() => setIdea("")} aria-label="Clear idea">×</button></div>
