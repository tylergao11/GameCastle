...
function materializeCandidate(s, candidate) {
  if (s.projectAssetDir && s.cloudAssetEngine && candidate.assetId && typeof s.cloudAssetEngine.materialize === 'function') { var expectedSemanticTags = candidate.semanticTags, expectedStyleTags = candidate.styleTags, expectedStyleId = candidate.styleId, localized = s.cloudAssetEngine.materialize({ requestId: s.runId + ':materialize:' + s.slot.slotId, revisionId: candidate.revisionId || candidate.assetId, projectId: s.projectId || s.runId, targetScope: 'project-local', projectAssetDir: s.projectAssetDir }); return Object.assign(candidate, localized, { status: candidate.status, source: candidate.source, path: localized.path, styleId: expectedStyleId, semanticTags: expectedSemanticTags, styleTags: expectedStyleTags, materialized: true }); }
...
function cloudPromotionAsset(s) { var asset=Object.assign({},s.candidate,{kind:s.source.cloudKind||'raster',semanticTags:s.source.semanticTags||s.candidate.semanticTags||s.slot.semanticTags||[],provenanceTypeId:s.source.provenanceTypeId||s.candidate.provenanceTypeId||null,licensePolicyId:s.source.licensePolicyId||s.candidate.licensePolicyId||null,qualityTierId:s.source.qualityTierId||s.candidate.qualityTierId||null,qualityFlags:s.source.qualityFlags||s.candidate.qualityFlags||[],templateSlots:s.source.templateSlots||[],parentRevisionId:s.candidate.parentRevisionId||null}); delete asset.styleTags; delete asset.provenance; delete asset.license; delete asset.quality; return asset; }
function lookupCloud(s) {
  if (!s.cloudAssetEngine) return null;
  try {
    var exact = typeof s.cloudAssetEngine.findExactForSpec === 'function' ? s.cloudAssetEngine.findExactForSpec(s.slot) : null;
    if (cloudAssetIsApproved(exact)) return { kind: 'cloud_exact', asset: exact, localPlan: exact.localPlan || null };
    var near = typeof s.cloudAssetEngine.findNearForSpec === 'function' ? s.cloudAssetEngine.findNearForSpec(s.slot) : null;
    if (cloudAssetIsApproved(near)) return { kind: 'cloud_near', asset: near, localPlan: near.localPlan || null };
  } catch (error) { s.cloudLookupError = error.code || 'CLOUD_QUERY_UNAVAILABLE'; }
  return null;
}
async function loadLangGraph() { return import('@langchain/langgraph'); }

async function runSlot(input) {
  var lg=await loadLangGraph(); var ledger=input.ledger||{}; var initial={runId:input.runId,projectId:input.projectId||input.runId,slot:input.slot,spec:input.spec||input.slot,sources:input.sources||{},localAssets:input.localAssets||{},cloudAssetEngine:input.cloudAssetEngine||null,visualIntents:input.visualIntents||{},ports:input.ports||{},modelPolicy:input.modelPolicy||null,ledger:ledger,projectAssetDir:input.projectAssetDir||null,maxAttempts:input.maxAttempts||2,maxCost:input.maxCost===undefined?Infinity:input.maxCost,trace:[]};
  ...
  .addNode('deterministic-variant',node('deterministic-variant',async function(s){if(s.source.needsPixels){s.decision='image_edit';return;} if(s.source.localPlan&&s.source.localPlan.requiresNewPixels!==true&&s.ports.localPlan&&typeof s.ports.localPlan.run==='function'){s.source.asset=materializeCandidate(s,Object.assign({},s.source.asset||{}));var planned=await s.ports.localPlan.run(s);s.candidate=record(s,'deterministicVariant','variant',planned);return;} if(s.source.derivationSpec&&s.ports.localDerive&&typeof s.ports.localDerive.derive==='function'){var derived=await s.ports.localDerive.derive(s);s.candidate=record(s,'deterministicVariant','variant',derived);return;} if(typeof s.ports.variant==='function'){var variant=await s.ports.variant(s);s.candidate=record(s,'deterministicVariant','variant',variant);return;} s.candidate=materializeCandidate(s,record(s,'deterministicVariant','variant',s.source.asset||{}));}))
  ...
  .addNode('asset-finalize',node('asset-finalize',function(s){var e=s.ledger[key(s)];if(e)e.completed={candidate:clone(s.candidate),review:clone(s.review)};s.cloudPromotionQueue=s.accepted&&s.source.requestCloudPromotion?[{requestId:s.runId+':promotion:'+s.slot.slotId,slotId:s.slot.slotId,assetId:s.candidate.assetId,parentRevisionId:s.candidate.parentRevisionId||null,asset:cloudPromotionAsset(s),receipt:{accepted:true,runId:s.runId}}]:[];s.assetManifest=manifest(s);s.assetWorld=assetWorld.buildAssetWorld(s.assetManifest,null);}))
