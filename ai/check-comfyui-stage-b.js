...
(async function() {
  ...
  try {
    ...
    var review = await ports.review(Object.assign({}, state, { candidate: edited, source: { reviewPolicy: { requiredSemanticTags: ['hero'], minConfidence: 0.35 } } })); assert.equal(review.pass, true); assert.equal(review.reviewer, 'florence2-semantic-review');
    assert.equal(prompts[0]['5'].class_type, 'VAEEncodeForInpaint'); assert.equal(prompts[1]['3'].class_type, 'Florence2Run'); assert.equal(prompts[1]['4'].class_type, 'SaveText');
    ...
    console.log('[ComfyUIStageB] typed parent/mask edit, immutable child provenance, Florence review, protected pixels and stale refs passed');
