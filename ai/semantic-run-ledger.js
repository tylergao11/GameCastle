...
function signature(command) { return JSON.stringify(stable(command)); }
function normalize(value) { return String(value || '').replace(/\s+/g, ' ').trim().toLowerCase().replace(/\d+/g, '#').slice(0, 240); }
function create(request) { return { schemaVersion: 1, ledgerKind: 'semantic-run-ledger', request: String(request || ''), status: 'running', completed: {}, failed: {}, failureCounts: {}, rounds: [] }; }
function completedSummaryFor(ledger, command) { var item = ledger.completed[signature(command)]; return item ? item.summary : null; }
