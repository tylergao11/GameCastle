...
(async function() {
  ...
  assert.strictEqual(spatialEngine.contract.activationGates.geometryFactProducer.status, 'required-before-plan', 'Native geometry blocks planning until a canonical producer exists.');
  ...
  try {
    var playerBytes = png.encodePng({ width: 2, height: 2, data: Buffer.from([64, 192, 255, 255, 64, 192, 255, 255, 64, 192, 255, 255, 64, 192, 255, 255]) });
    var hudBytes = png.encodePng({ width: 2, height: 2, data: Buffer.from([255, 192, 64, 255, 255, 192, 64, 255, 255, 192, 64, 255, 255, 192, 64, 255]) });
    var playerPath = path.join(root, 'player.png'), hudPath = path.join(root, 'hud.png');
    fs.writeFileSync(playerPath, playerBytes); fs.writeFileSync(hudPath, hudBytes);
    var assetWorld = {
      schemaVersion: 2,
      documentKind: 'semantic-asset-world',
      sourceHash: assembly.sourceHash,
      contentHash: 'asset-world.spatial-planner.fixture',
      slots: [
        { semanticId: 'player_visual', path: playerPath, sha256: crypto.createHash('sha256').update(playerBytes).digest('hex'), format: 'png', width: 2, height: 2, resourceKind: 'image', transparent: false },
        { semanticId: 'hud_visual', path: hudPath, sha256: crypto.createHash('sha256').update(hudBytes).digest('hex'), format: 'png', width: 2, height: 2, resourceKind: 'image', transparent: false }
      ]
    };
    var assetBound = binder.bindResources(assembly.projectSeed, assetWorld);
    var geometryFacts = seal({
      schemaVersion: 2,
      documentKind: 'spatial-geometry-fact-set',
      sourceHash: assembly.sourceHash,
    ...
    var product = await spatialProduct.run({
      ...
      assetProduct: { documentKind: 'semantic-asset-product', source: source, assembly: assembly, artifact: assetBound, assetState: { assetWorld: assetWorld } },
