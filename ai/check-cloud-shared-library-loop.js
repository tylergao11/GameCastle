...
try {
  var sharedRoot = path.join(root, 'shared-cloud'), producerProject = path.join(root, 'producer-project'), consumerProject = path.join(root, 'consumer-project');
  ...
  var producer = engineModule.createCloudAssetEngine({ rootDir: sharedRoot });
  producer.enqueuePromotion({ cloudPromotionQueue: [{ requestId: 'producer.hero.v1', asset: { assetId: 'producer.hero', path: source, kind: 'raster', format: 'png', width: 1, height: 1, transparent: true, styleId: 'gamecastle.style-1', semanticTags: ['role.hero'], provenanceTypeId: 'provenance.user-final', licensePolicyId: 'license.creator-share', qualityTierId: 'quality.accepted', qualityFlags: [] }, receipt: { accepted: true }, runtimeBindingReceipt: { slotId: 'hero', status: 'bound' }, shareConsent: true }] });
  assert.equal(producer.sync()[0].state, 'published');
  var consumer = engineModule.createCloudAssetEngine({ rootDir: sharedRoot });
  var candidate = consumer.query({ requestId: 'consumer.query.hero', assetSpec: { styleId: 'gamecastle.style-1', semanticTags: ['hero'] }, templateContext: {}, localCapabilities: {}, policy: {} }).candidates[0];
