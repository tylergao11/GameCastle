...
var source = { schemaVersion: 4, documentKind: 'game-semantic-source', dictionarySource: index.source, game: { semanticId: 'frame_set_binding_demo', name: 'Frame Set Binding Demo' }, entities: [{ semanticId: 'player', roles: ['player'], objectTypeRef: 'gdjs://object/Sprite::Sprite', behaviorTypeRefs: [], members: [] }], events: [], assetIntents: [{ semanticId: 'player_animation', roles: ['player', 'visual'], subject: 'player', description: 'A readable player movement animation.', productionFamily: 'character', styleId: 'gamecastle.style-dna.v1', constraints: { transparent: true }, bindings: [] }], layoutIntents: [], tuningPolicies: { relativeChange: { slight: { mode: 'percentage', value: 0.1 } } } };
...
try {
  ...
  assert.throws(function() { binder.bind(seed, Object.assign({}, world, { slots: [{ semanticId: 'player_animation', frameSet: uneven }] })); }, function(error) { return error.code === 'FRAME_SET_GDJS_VARIABLE_TIMING_UNSUPPORTED'; });
