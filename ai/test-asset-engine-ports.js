...
function createTestAssetEnginePorts(options) {
  ...
  return {
    generate: async function(state) {
      ...
      return { assetId: 'test.' + sha256.slice(0, 16), revisionId: 'test-revision.' + sha256.slice(0, 16), sha256: sha256, path: file, format: 'png', width: width, height: height, transparent: true, semanticTags: (slot.semanticTags || []).slice(), styleTags: (slot.styleTags || []).slice(), styleId: slot.styleId || 'gamecastle.style-dna.v1', status: 'generated', provenance: 'test-only-port', publishability: { playable: true, publishable: false, blocksFinalExport: false } };
    },
    review: async function() { return { pass: true, repairable: false, issues: [], reviewer: 'test-only-port' }; },
