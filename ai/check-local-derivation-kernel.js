...
var spec = { schemaVersion: 1, dictionaryId: 'gamecastle.asset-style-dictionary', styleId: 'gamecastle.style-dna.v1', operationId: 'op.sheet.1', op: 'sprite_sheet_split', input: { assetId: 'asset.sheet', contentHash: 'sha256.input' }, params: { columns: 3 }, output: { format: 'png', transparent: true }, scope: 'project-local' };
(async function() {
  ...
  assert.equal(called, 1); assert.equal(receipt.owner, 'LocalDerivationKernel'); assert.equal(receipt.outputHash, 'sha256.output'); assert.equal(receipt.scope, 'project-local'); assert.equal(receipt.styleId, 'gamecastle.style-dna.v1');
