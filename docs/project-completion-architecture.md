...
2. WP1 接真实 Provider；没有凭据时仍 fail closed，不阻断本地 deterministic 路径。
3. WP2 提供足够的轻量玩法模块和模板，使多数点子无需临时生成玩法代码。
4. WP3–WP4 把引擎能力变成简单、连续、有版本的玩家体验。
...
当前 `project-pipeline-graph.js` 中 World Intent 节点是 `wired-langgraph`，其他许多节点仍是
`wired-langgraph-smoke`。WP0 完成后，所有项目节点必须在一次正式运行中读写契约状态、产生真实
artifact、支持 checkpoint/resume，并通过 owner failure matrix。只有模块存在或 smoke 通过不能把
节点改为 live。
