...
| `npm run check:comfyui-stage-b` | 受控 parent/mask ImageEdit、mask 外像素保护、private workflow gate、派生 candidate taint、语义验收交接与 child revision persistence | mock Comfy transport；禁止真实模型/网络 | 已通过；Florence 由独立 loopback smoke 验证 |
...
| `npm run smoke:comfyui:live` | 真实本地 ComfyUI 生成一张 256×256 透明 PNG | 仅显式手工调用；要求真实本地 ComfyUI 和固定模型 hash | 验证生成、透明输出和 provenance，不代表质量、并发或 C-2 evidence |
