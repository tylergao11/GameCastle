...
| `npm run smoke:comfyui-local` | 一张真实本机 ComfyUI 生成进入 AssetWeave、review、validation、acceptance、revision 与 binding | 显式 `COMFYUI_ALLOW_LOCAL=true`，受预算/超时限制 | 2026-07-13 已用 CPU 通过一次真实非 fixture PNG；不代表生产性能 |
...
| `npm run check:comfyui-worker` | 受 governance 的云 Worker health/submit/status/cancel、deployment attestation、固定 output fetch、response body deadline、私有输入拒绝、AssetWeave binding、accepted Worker→AssetRevision provenance | 注入 mock Worker；禁止真实 GPU/网络；deployment registry 在测试外保持未批准 | Stage C 启动基线已实现；不是 GPU 生产验证 |
...
| `npm run smoke:comfyui-stage-b` | 真实生成父图 → 受控 inpaint → Florence-2 review → 子 AssetRevision → binding | 显式本地授权、固定 Florence model/custom-node hashes | 2026-07-13 CPU 通过；非生产能力 |
