...
Director Planner coordinates only the cross-domain `semantic.design`, `asset.realize`, and `assembly.verify` APIs. Its default path freezes the canonical `director-dsl-v1` program locally, invokes no model, and reuses one compiled LangGraph. Semantic Planner owns semantic decomposition, and DSL Executor fills values for exactly one frozen active task. Both consume the same DSL syntax registry through separate model roles on the current open-source Semantic model. Runtime owns GDJS capability binding. After accepted assets exist, Spatial Planner owns visual arrangement within those frozen facts. The local semantic runtime commits accepted task batches and materializes the complete `GameSemanticSource` or source-hash-checked `GameSemanticRevision`.

Model selection is supplied through ports. The current Semantic adapter is pinned CUDA llama.cpp on port `8002`, serving the `Qwen/Qwen3.5-9B` `UD-Q4_K_XL` GGUF from a persistent Docker volume. Every Semantic request passes `enable_thinking=false` and its phase-specific GBNF. simulated-local supports deterministic tests without changing Planner, Executor, Parser, Runtime, or training-log contracts. Ollama remains only the configured Spatial vision adapter. A separate `Qwen/Qwen3-4B-Instruct-2507` Director endpoint on port `8001` is reserved for explicit `dynamicPlanning: true`; it is not downloaded, started, or required by the deterministic default path.
...
Director Planner sees one typed operation registry: `semantic.design`, `asset.realize`, and `assembly.verify`. These API boundaries return structured domain results and receipts. Director state stores only cross-domain dependencies, receipts, accepted hashes, and failure facts. Semantic slots, asset work items, assembly candidates, provider messages, and parser repairs remain inside their owning domains, keeping LangGraph orchestration state smaller than the combined domain harness states. The default canonical plan produces a deterministic receipt and no provider cost. Only explicit dynamic-planning mode invokes the Director model; if such a paid response fails DSL validation, its receipt is reconciled into `ProductDeliveryRun` cost history before the run is terminally blocked.
...
Product request + Planner discovery catalog
  ...
  -> Director Planner locally freezes canonical director-dsl-v1: semantic.design -> asset.realize -> assembly.verify
