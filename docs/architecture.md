...
Model selection belongs to the consuming domain, never the shared ProviderRuntime. `director-model-port.js` pins Director LLM1 to DeepSeek `deepseek-v4-flash`. `semantic-model-policy.js` owns Semantic LLM2: `GAMECASTLE_RUNTIME_MODE=production` (default) pins CUDA llama.cpp with `Qwen/Qwen3.5-9B`; `development` routes LLM2 to the same DeepSeek transport for local iteration without the GPU service. `.env.local` supplies mode plus machine-private endpoints, keys, and local authorization. Every Semantic request passes `enable_thinking=false` and its phase-specific GBNF (server-enforced only on llama.cpp). simulated-local is only a test double and does not form an alternate production Planner path. Ollama remains only the configured Spatial vision adapter.
