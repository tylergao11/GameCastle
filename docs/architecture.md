...
Planner and Executor system prompts are versioned profiles (`semantic-planner-prompt-v24`, `semantic-executor-prompt-v30`). Planner is dispatch-only (`plan-task` / `plan-complete`); Executor free-writes the active work order. Executor layout: **system** holds law, mode-legal FORMS, and dictionary catalogs (`L1-structure-kinds`, `L1-ops-*`); **user** holds only `[L3-work-order]`, `[L3-board]` (with `workMode` and member values), optional product background, and L4. Revision FORMS/GBNF omit `game`/`policy`. Model `capability=` is a foundation handle (not plan-use aliases). Open fields carry parameters; nested `arguments=` is rejected. Expression parameters accept a bare `Owner.field` as `state.number` / `state.text` sugar. The prompt contains no examples and no JSON response format; GBNF from `semantic-dsl-v9` constrains wire shape on llama.cpp. Parser and Runtime own semantic validity.
