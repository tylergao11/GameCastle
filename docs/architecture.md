...
**One production delivery authority:** `ProductDeliveryOrchestrator` + persisted `ProductDeliveryRun` (HTTP `POST /product/deliver`). Sealed stage DAG is Director (`director-dsl-v1`): `semantic.design -> asset.realize -> assembly.verify` with assembly rejection → semantic revision. Pin: `packages/product/src/product-truth.js`.

**Product-dispatch LangGraph** (`product-dispatch-*`) is a **lane scheduler scaffold** only (placeholder ledger, optional parallel semantic|asset after seal, assembly **gate**). It must not mint a second accepted product identity outside `ProductDeliveryRun`. Placeholder statuses: `declared` → `sealed` (intent settled) → `filled` (asset accepted); assembly is gate-driven, not a model work order.

Semantic Planner is a **domain-internal** dispatcher only (`plan-task` / `plan-complete`), not the product total scheduler. DSL Executor free-writes the active semantic task. Runtime owns GDJS capability binding. After accepted assets exist, Spatial Planner owns visual arrangement within those frozen facts. The local semantic runtime commits accepted task batches and materializes the complete `GameSemanticSource` or source-hash-checked `GameSemanticRevision`.

Model selection belongs to the consuming domain, never the shared ProviderRuntime. `director-model-port.js` pins Director LLM1 to DeepSeek `deepseek-v4-flash`. `semantic-model-policy.js` owns Semantic LLM2: `GAMECASTLE_RUNTIME_MODE=production` (default) pins local Ollama `qwen3:8b`; `development` routes LLM2 to the same DeepSeek transport for iteration without local models. Spatial vision uses the same Ollama runtime with `qwen3-vl:8b`. `.env.local` supplies mode plus private endpoints, keys, and `OLLAMA_ALLOW_LOCAL`. Semantic requests keep thinking disabled and may attach a phase-specific GBNF field for clients that honor it; Runtime parser remains authoritative (Ollama does not server-enforce GBNF). simulated-local is only a test double.

Runtime owns deterministic mechanics: TaskPlan feasibility and sealing, one-task activation, `authorizeWriteBatch` (write order, foundation-handle resolve, scope, lifecycle reseed), dictionary binding, official parameter ordering/default insertion, GDJS expression/token normalization, collision-safe internal reference generation, open-slot validation, scene/object variable routing, official recursive variable serialization, dictionary-declared event serialization, nested event construction, one-to-many operation expansion, task transactions with delta verification, Source/Revision materialization, and compilation. It does not choose gameplay or design values.

The semantic runtime follows one state-machine loop: `PLANNING → TASK_READY → TASK_ACTIVE → ... → FINALIZING → COMPLETED`, with repair, fuse, and expiry states on the same immutable event ledger. The Planner is dispatch-only: one `plan-task` work order or `plan-complete` per round. Runtime freezes that plan and activates the single task; the Executor free-writes structure and values against L1 catalogs and the L3 work-order/board. One write batch is authorized and committed atomically, or the fork is discarded. When free planning is allowed, plan-scoped write failures (empty delta/batch) clear the sealed plan and re-enter `PLANNING`; externally frozen plan DSL does not take that path. Two consecutive identical failure facts fuse; no second status or fuse counter exists.

Planner and Executor system prompts are versioned profiles (`semantic-planner-prompt-v24`, `semantic-executor-prompt-v31`). Planner is dispatch-only (`plan-task` / `plan-complete`); Executor free-writes the active work order. Executor layout: **system** holds law, mode-legal FORMS, and dictionary catalogs (`L1-structure-kinds`, `L1-ops-*`, `L1-asset-families`, `L1-asset-styles`, `L1-layouts`); **user** holds only `[L3-work-order]`, `[L3-board]` (with `workMode` and member values), optional product background, and L4. Revision FORMS/GBNF omit `game`/`policy`. Model `capability=` is a foundation handle (not plan-use aliases). Open fields carry parameters; nested `arguments=` is rejected. Expression parameters accept a bare `Owner.field` as `state.number` / `state.text` sugar. The prompt contains no examples and no JSON response format; GBNF from `semantic-dsl-v9` is advisory on Ollama. Parser and Runtime own semantic validity.
...
`ProductDeliveryOrchestrator` owns the cross-stage product loop; `ProductDeliveryRun` is the only persisted product lifecycle truth. The orchestrator classifies stage failures as local retry, semantic Revision, or system block. Only exact-target semantic quality observations become feedback. TaskPlan remains Semantic Planner's only mutation scope inside the semantic domain. Friend multiplayer admits rooms by delivery `sourceHash`; host (initiator) owns the input timeline.
...
Product request + Planner discovery catalog
  ...
  -> semantic.design: Semantic Planner semantic-dsl-v9 dispatch (one plan-task work order or plan-complete) -> feasibility + freeze
  -> state machine activates that single task
  -> Executor free-write -> authorizeWriteBatch (foundation handles) -> atomic commit or rollback (plan-scoped failures may replan)
...
`GameSemanticSource` v6 is strict and dictionary-pinned. It stores component instances beside entities, events, assets, and layout intent. A component instance contains one dictionary component reference, target, explicit `config`, and semantic bindings (Draft IR). On the model wire, Executor writes `component.capabilityBindings` as named capability bindings (foundation handles after resolve map to Draft `bindings`). Component authoring schema v3 has no parallel configuration shape. Defaults, inheritance, requirements, and implementation remain in the generated dictionary. Runtime expands the selected component into members, entities, behaviors, layouts, and events in private compilation material. The assembly exposes only a component-expansion evidence document with hashes, resolved configuration, and generated IDs; no second Source document crosses the boundary.
...
Exact event references are runtime output: LLM2 sees foundation entity, behavior, event, condition, action, expression, and complete component handles through L1/L3 projections. Every event invocation retains runtime provenance (`use`, stable operation identifier, expansion part and size), its exact instruction channel, condition inversion or action await, and canonical normalized arguments. Event nodes retain recursive local variables and children. Member, asset, and layout Source bindings are optional foundation operation-tag lists on the model wire (omit or `list()`); after resolve they retain stable use plus dictionary expansion where present. Member values use the official recursive GDJS variable types. Layouts, asset families/styles, extension groups, and retrieved extension object/behavior/event/operation kinds use compact handles. Internal `gdjs://...` and `gc-component://...` references remain runtime-only. Asset intents are compiled from the same Source as events and layout.
