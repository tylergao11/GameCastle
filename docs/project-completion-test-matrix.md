...
| WP1 Stage A — ComfyUI local | `check:comfyui-local` 覆盖 loopback health、submit/history、临时候选→验收后 project-local 晋升与 receipt provenance；`smoke:comfyui:live` 只验证一次真实透明 PNG 生成 | endpoint 缺失/不可用、timeout、cancel、坏 PNG、预算耗尽、重复 request | interrupt、placeholder debt、同一 ledger 不重复提交 | 契约检查与单次真实生成均可运行；真实 smoke 不声称生产性能或完整 WP1 |
