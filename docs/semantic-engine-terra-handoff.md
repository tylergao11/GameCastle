...
The Director model port is deliberately separate. It owns the external DeepSeek `deepseek-v4-flash` selection for LLM1; Semantic model policy owns the local open-source Qwen selection for LLM2. Shared ProviderRuntime owns neither choice. `npm run product:serve` loads only private endpoint and key values from `.env.local`. Ollama is not a Semantic fallback and remains scoped to Spatial vision configuration.
