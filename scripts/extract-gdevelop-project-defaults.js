...
var OUT = path.join(ROOT, 'ai', 'gdevelop-truth', 'project-defaults.json');
...
(async function() {
  ...
  if (CHECK) {
    if (!fs.existsSync(OUT) || fs.readFileSync(OUT, 'utf8') !== serialized) throw new Error('GDevelop project defaults drifted. Run node scripts/extract-gdevelop-project-defaults.js');
