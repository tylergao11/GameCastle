...
var OUT_PATH = path.join(ROOT, 'packages', 'gdjs', 'generated', 'event-grammar.json');
...
function main() {
  ...
  if (CHECK_MODE) {
    if (!fs.existsSync(OUT_PATH) || fs.readFileSync(OUT_PATH, 'utf8') !== output) throw new Error('GDJS event grammar snapshot is stale; run node scripts/gdevelop/extract-gdevelop-event-grammar.js');
