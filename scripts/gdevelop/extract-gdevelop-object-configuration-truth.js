...
var BINDINGS = path.join(ROOT, 'packages', 'gdjs', 'generated', 'official-capability-bindings.json');
var OUT = path.join(ROOT, 'packages', 'gdjs', 'generated', 'object-configuration-truth.json');
...
(async function() {
  ...
  if (CHECK) { if (!fs.existsSync(OUT) || fs.readFileSync(OUT, 'utf8') !== serialized) throw new Error('GDevelop object configuration truth drifted. Run node scripts/gdevelop/extract-gdevelop-object-configuration-truth.js'); console.log('[GDevelopObjectConfigurationTruth] snapshot OK: ' + records.length + ' official object types'); return; }
