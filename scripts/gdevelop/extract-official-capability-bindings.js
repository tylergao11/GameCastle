...
var OUT = path.join(ROOT, 'packages', 'gdjs', 'generated', 'official-capability-bindings.json');
...
async function main() {
  ...
  if (process.argv.indexOf('--check') >= 0) {
    if (!fs.existsSync(OUT) || fs.readFileSync(OUT, 'utf8') !== serialized) throw new Error('Official capability bindings drifted; run node scripts/gdevelop/extract-official-capability-bindings.js');
