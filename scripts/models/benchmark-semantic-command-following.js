...
var cases = [
  ...
  { name: 'component', phase: 'executor', prompt: '只输出一条 component 命令：slot=movement，kind=topdown-movement，targetSlot=hero，config 是 speed=200 的 record，capabilityBindings 是空 record。', expected: { type: 'component', slot: 'movement', kind: 'topdown-movement', targetSlot: 'hero', config: { speed: 200 }, capabilityBindings: {} } },
  ...
  { name: 'when', phase: 'executor', prompt: '只输出一条 when 命令：slot=collision，capability=hit，other=enemy；参数作为 open fields，不要用 arguments=。', expected: { type: 'when', slot: 'collision', capability: 'hit', other: 'enemy' } },
