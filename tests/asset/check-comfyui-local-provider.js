...
(async function() {
  ...
  try {
    ...
    var model = path.join(root, 'sdxl-base.safetensors'), refiner = path.join(root, 'sdxl-refiner.safetensors'); fs.writeFileSync(model, Buffer.from('sdxl-base-test-checkpoint')); fs.writeFileSync(refiner, Buffer.from('sdxl-refiner-test-checkpoint'));
  ...
  } finally { semanticReviewer.reviewImages = savedReviewImages; semanticReviewer.fingerprint = savedFingerprint; semanticReviewer.receipt = savedReceipt; restore(saved); fs.rmSync(root, { recursive: true, force: true }); }
