...
(async function() {
  ...
  try {
    ...
    assert.strictEqual(prewarmCalls, 1, 'API startup begins non-blocking graph prewarm before the first product request.');
    ...
    assert.strictEqual(sourceRejected.status, 400, 'HTTP product delivery owns initial semantic design and cannot accept a caller-authored Source.');
    var creativeVisionRejected = await fetch(base + '/product/deliver', { method: 'POST', headers: headers(), body: JSON.stringify({ deliveryId: 'delivery-api', projectId: 'project-api', userRequest: 'Build it.', creativeVision: 'Readable.' }) });
    assert.strictEqual(creativeVisionRejected.status, 400, 'HTTP delivery no longer accepts a caller-authored creative-model direction.');
    ...
    var productResponse = await fetch(base + '/product/deliver', { method: 'POST', headers: headers(), body: JSON.stringify({ deliveryId: 'delivery-api', projectId: 'project-api', userRequest: 'Build it.' }) });
    ...
    assert.deepStrictEqual(productInput, { deliveryId: 'delivery-api', projectId: 'project-api', userRequest: 'Build it.' });
    var blockedResponse = await fetch(base + '/product/deliver', { method: 'POST', headers: headers(), body: JSON.stringify({ deliveryId: 'blocked-api', projectId: 'project-api', userRequest: 'Build it.' }) });
