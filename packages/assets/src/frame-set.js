...
var contract = require('../contracts/frame-set-contract.json');
...
function normalizeCore(input) {
  ...
  return { schemaVersion: contract.schemaVersion, resourceKind: contract.resource.resourceKind, format: contract.resource.format, initialStateId: input.initialStateId, canvas: { width: input.canvas.width, height: input.canvas.height }, anchor: { x: input.anchor.x, y: input.anchor.y }, frames: frames, states: states, provenance: clone(input.provenance || {}), semanticReviewReceipt: clone(input.semanticReviewReceipt || null) };
}
