...
var dictionaryApi = require('../semantic/src/capability-semantic-dictionary');
var sourceContract = require('../semantic/src/game-semantic-source');
var componentExpander = require('../semantic/src/component-expander');
var semanticCompiler = require('../semantic/src/semantic-compiler');
var assetCompiler = require('../semantic/src/semantic-asset-compiler');
var layoutCompiler = require('../semantic/src/semantic-layout-compiler');
...
// Load the generated semantic dictionary once so every public operation in this
// process validates and compiles against the same pinned source fingerprint.
...
function bindCompilerOutput(output, name, sourceHash, realizedSourceHash, dictionarySource, contentHashPrefix) {
  if (output.sourceHash !== realizedSourceHash) {
    throw new Error('Semantic module received ' + name + ' output from a different realized GameSemanticSource');
  }
  if (!same(output.dictionarySource, dictionarySource)) {
    throw new Error('Semantic module received ' + name + ' output from a different GDJS Semantic Dictionary');
  }
...
function compileSemanticAssembly(source) {
  ...
  if (expansion.sourceHash !== sourceHash || expansion.realizedSourceHash !== realizedSourceHash) {
    throw new Error('Semantic module received a mismatched component expansion');
  }
