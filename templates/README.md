...
GameCastle 的目标是模块化游戏工厂。后续这里应该演进为能力模块库，描述可组合能力，而不是描述一整套游戏。
...
LLM1 只应该看到轻量能力提示，例如“支持平台跳跃、射击、收集、敌人、Boss、联机雏形”。它不应该看到模板结构、完整能力 schema 或 DSL 示例。

LLM2 可以看到模块能力 schema、DSL 示例和参数约束，并负责把 LLM1 的高层意图编译成 patch。
